...
import { X, MapPin, CreditCard, Truck, AlertCircle, Plus, RefreshCw, Trash2 } from "lucide-react"; // ✅ added Trash2
...
export default function CheckoutModal({ isOpen, onClose, onOrderPlaced }) {
  ...
  const { addresses, loading: addressLoading, error: addressError, fetchAddresses, createAddress, deleteAddress } = useAddresses(false);
  ...
  const handleCreateAddress = async (e) => {
    ...
    setError("");
    // Validation
    if (!newAddress.full_name.trim()) { setError("Full name is required"); return; }
    if (!newAddress.phone.trim() || !/^[0-9]{10}$/.test(newAddress.phone)) { setError("Valid 10-digit phone number is required"); return; }
    if (!newAddress.line1.trim()) { setError("Address line 1 is required"); return; }
    if (!newAddress.city.trim()) { setError("City is required"); return; }
    if (!newAddress.state.trim()) { setError("State is required"); return; }
    if (!newAddress.postal_code.trim() || !/^[1-9][0-9]{5}$/.test(newAddress.postal_code)) { setError("Valid 6-digit PIN code is required"); return; }
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          animate={{ scale: 1, y: 0 }}
          exit={{ scale: 0.9, y: 20 }}
          className="bg-[#0d0d0d] rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-white/10 shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="sticky top-0 bg-[#0d0d0d] z-10 flex items-center justify-between p-6 border-b border-white/10">
                  ...
                  {addressLoading ? (
                    ...
                    <>
                      {addresses.length > 0 ? (
                        ...
                        <p className="text-gray-400 text-sm">No saved addresses found. Please add a new address.</p>
                      )}
